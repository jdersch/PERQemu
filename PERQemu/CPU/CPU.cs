// cpu.cs - Copyright 2006-2016 Josh Dersch (derschjo@gmail.com)
//
...
namespace PERQemu.CPU
{
    ...
    public sealed class PERQCpu
    {
        ...
        /// <summary>
        /// "Real" clock speed in cycles/sec (based on 170ns cycle time).  Approx. 5.9Mhz.
        /// </summary>
        ...
        /// than the Z80 on the IOB, but clocking the IO even every other cycle is a lot of
        /// mostly wasted overhead.  Make this more explicit and tunable here (8-16 is a
        /// pretty good range?) and expose it so the "clock" hardware can simulate the 60Hz
        /// line frequency "jiffy clock" more accurately.  Cuz why not.
        /// </summary>
        ...
        public RunState Execute(RunState currentState)
        {
            ...
            do
            {
                ...
                // Clock the memory state machine and do any pending fetches.
                _memory.Tick();
                ...
                //
                waiting = (_wcsHold || _memory.Wait ||
                          (_memory.TState <= 1 && uOp.WantMDI && !_memory.MDIValid));
            ...
            if (curOp == ALUOperation.AplusB || curOp == ALUOperation.AminusB)
            {
                switch (_rasterOp.MulDivInst)
                {
                    case MulDivCommand.Off:
                        // Should never happen!  (Pass thru unmodified op... rather than throw exception?)
#if TRACING_ENABLED
                        test = mdr;
                        Console.WriteLine("WTF? DoMulDivALUOp called with MulDivInst=OFF?");
#endif
                        break;
                ...
                case 0x1:   // Store / Extended functions
                    switch (uOp.SF)
                    {
                        ...
                        case 0x1:   // Multiply / DivideStep
                            ...
                            // here we take care of the low word in the MQ register.  We'll just make it work
                            // now, worry about making it elegant later, if that's even possible...
                            //
#if TRACING_ENABLED
                            if (Trace.TraceOn)
                            ...
                            switch (_rasterOp.MulDivInst)
                            {
                                case MulDivCommand.Off:
                                    // Should never happen...
                                    throw new UnimplementedInstructionException("MulDiv error: step SF while not enabled??");
                        ...
                        case 0xe:   // Fetch
                        case 0xf:   // Store
                            _memory.RequestMemoryCycle(uOp.MemoryRequest, _alu.Registers.R, _clocks);
                            break;

                        default:
                            throw new UnimplementedInstructionException(
                                String.Format("Unimplemented Storage Function {0:x1}", uOp.SF));
                    }
                    break;
        ...
        private void ShowMemory(uint startAddress, uint length)
        {
            ...
            if (startAddress >= Memory.MemoryBoard.Instance.Memory.Length ||
                endAddr - 8 >= Memory.MemoryBoard.Instance.Memory.Length)
            {
                Console.WriteLine("Argument out of range -- must be between 0 and {0}", Memory.MemoryBoard.Instance.Memory.Length);
                return;
            }

            ushort[] mem = Memory.MemoryBoard.Instance.Memory;
        ...
        private void ShowOpfile()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                Console.WriteLine("{0}: {1:x2}", i, Memory.MemoryBoard.Instance.OpFile[i]);
            }
        }
